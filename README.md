...
先看图标包还挂着没有。升级 App 或者重装之后，桌面有时会退回系统图标，去桌面设置里把「奶蛙图标」重新选一次就恢复了；换成别的图标包再换回来也一样。

重选过还是没变，多半是这一枚没收录。图标包模式靠一张「应用 → 图标」的映射表匹配，应用改版换了启动入口，或者入口本来就不常见，就会漏掉。把「导出应用清单」发过来，下一版补上；也可以先用快捷方式模式单独换它。
